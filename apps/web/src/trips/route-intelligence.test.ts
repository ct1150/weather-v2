...
const hotel: RouteAnchor = { id: "hotel", label: "Hotel", latitude: 35.68, longitude: 139.76 };
...
describe("route intelligence", () => {
  it("optimizes movable waypoints while preserving a locked waypoint position", () => {
    ...
    expect(new Set(optimized.map((item) => item.id))).toEqual(new Set(input.map((item) => item.id)));
  });

  it("is deterministic for identical input", () => {
    const input = [
      waypoint("b", 35.70, 139.79),
      waypoint("a", 35.70, 139.79),
      waypoint("c", 35.71, 139.80),
    ];
    expect(optimizeRouteOrder(input, { start: hotel })).toEqual(optimizeRouteOrder(input, { start: hotel }));
  });

  it("creates an estimated route with hotel bookends", () => {
    const plan = estimateRoutePlan(
      [waypoint("one", 35.69, 139.77), waypoint("two", 35.70, 139.78)],
      "driving",
  ...
  it("parses a routed OSRM plan", async () => {
    ...
    const plan = await fetchRoutedPlan([waypoint("one", 35.69, 139.77)], { start: hotel }, { fetchImpl });
  ...
  it("builds a source-to-destination route cost matrix", async () => {
    ...
    const matrix = await fetchRouteCostMatrix(
      ...
      [waypoint("d1", 35.70, 139.78), waypoint("d2", 35.71, 139.79)],
      { fetchImpl },
    );
