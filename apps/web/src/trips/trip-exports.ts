...
function compactLocalDateTime(value: Date): string {
  const iso = value.toISOString();
  return `${iso.slice(0, 10).replace(/-/gu, "")}T${iso.slice(11, 19).replace(/:/gu, "")}`;
}

function activityEndDateTime(
  date: string,
  startTime: string,
  endTime: string | null,
  ...
  if (endTime !== null) {
    ...
    return compactLocalDateTime(end);
  }
  start.setUTCMinutes(start.getUTCMinutes() + durationMinutes);
  return compactLocalDateTime(start);
}
  ...
  workspace.days.forEach((day) => {
    ...
    activities.forEach((activity) => {
      ...
      if (activity.startTime === null) {
        ...
        const start = parseLocalDateTime(day.date, activity.startTime);
        lines.push(`DTSTART:${compactLocalDateTime(start)}`);
        lines.push(
          `DTEND:${activityEndDateTime(
            day.date,
            activity.startTime,
            activity.endTime,
...
function packingBase(locale: TripExportLocale): PackingItem[] {
  if (locale === "en") {
    return [
      {
        id: "documents",
        label: "Passport / tickets / booking confirmations",
        reason: "Core travel documents",
        category: "documents",
      },
      {
        id: "power",
        label: "Chargers and power bank",
        reason: "Maps and weather execution depend on phone battery",
        category: "comfort",
      },
    ];
  }
...
function localizedItem(
  ...
  if (locale === "en") {
    const map: Record<string, PackingItem> = {
      "rain-shell": {
        id,
        label: "Light rain shell / compact umbrella",
        reason: `Peak rain probability ${numeric}%`,
        category: "weather",
      },
      "waterproof-bag": {
        id,
        label: "Waterproof pouch / phone bag",
        reason: "Protect documents and electronics",
        category: "weather",
      },
      "sun-protection": {
        id,
        label: "Sunscreen, hat and sunglasses",
        reason: `Peak UV ${numeric}`,
        category: "weather",
      },
      heat: {
        id,
        label: "Water bottle, electrolytes and quick-dry clothing",
        reason: `High around ${numeric}°C`,
        category: "weather",
      },
      "warm-layer": {
        id,
        label: "Light warm layer / wind shell",
        reason: `Low around ${numeric}°C`,
        category: "weather",
      },
      wind: {
        id,
        label: "Wind shell and secure hat strap",
        reason: `Peak gust around ${numeric} km/h`,
        category: "weather",
      },
      family: {
        id,
        label: "Children's spare clothing, snacks and usual medicines",
        reason: "Extra redundancy for family travel",
        category: "comfort",
      },
      senior: {
        id,
        label: "Regular medicines, insulated bottle and portable seat pad",
        reason: "Prioritize comfort and medication continuity",
        category: "comfort",
      },
    };
    return map[id]!;
  ...
  if (locale === "zh-hant") {
    const map: Record<string, PackingItem> = {
      ...
      "sun-protection": {
        id,
        label: "防曬霜、遮陽帽、太陽眼鏡",
        reason: `最高 UV ${numeric}`,
        category: "weather",
      },
      heat: {
        id,
        label: "水壺 / 電解質補充 / 快乾衣",
        reason: `最高溫約 ${numeric}°C`,
        category: "weather",
      },
      "warm-layer": {
        id,
        label: "輕薄保暖層 / 防風外套",
        reason: `最低溫約 ${numeric}°C`,
        category: "weather",
      },
      wind: {
        id,
        label: "防風外套與固定帽帶",
        reason: `最大陣風約 ${numeric} km/h`,
        category: "weather",
      },
      family: {
        id,
      ...
      senior: {
        id,
        label: "長者常用藥、保溫水杯、便攜坐墊",
        reason: "長者同行優先舒適與用藥連續性",
        category: "comfort",
      },
    };
    return map[id]!;
  ...
  const map: Record<string, PackingItem> = {
    ...
    heat: {
      id,
      label: "水壶 / 电解质补充 / 速干衣",
      reason: `最高温约 ${numeric}°C`,
      category: "weather",
    },
    "warm-layer": {
      id,
      label: "轻薄保暖层 / 防风外套",
      reason: `最低温约 ${numeric}°C`,
      category: "weather",
    },
    wind: {
      id,
      label: "防风外套与固定帽带",
      reason: `最大阵风约 ${numeric} km/h`,
      category: "weather",
    },
    family: {
      id,
    ...
    senior: {
      id,
      label: "老人常用药、保温水杯、便携坐垫",
      reason: "老人同行优先舒适与用药连续性",
      category: "comfort",
    },
  };
  return map[id]!;
...
export function buildWeatherPackingList(
  ...
  const maxWind = Math.max(
    0,
    ...forecasts.map((item) => item.windGustKph ?? item.windSpeedKph ?? 0),
  );
...
export function workspaceToPrintableHtml(
  ...
  const forecastByKey = new Map(
    forecasts.map((item) => [`${item.cityId}:${item.date}`, item] as const),
  );
  const dayHtml = workspace.days
    .map((day) => {
      ...
      const activityHtml = activities
        .map(
          (activity) =>
            `<li><strong>${htmlEscape(activity.startTime ?? "—")}</strong> ${htmlEscape(activity.title)}${activity.flexibility === "fixed" || activity.reservation === "required" ? " 🔒" : ""}</li>`,
        )
        .join("");
