...
export async function generateMetadata({
  ...
  if (country === undefined) return { title: "国家周末旅行天气" };
  const name = country.name["zh-cn"];
  const path = `/${country.slug}/best-weekend`;
  const title = `${name}本周末哪里天气好？周六周日基本不下雨城市排行 | Where Not Rain`;
  const description = `比较${name}热门旅行地即将到来的周六、周日基本不下雨天数、预计总降雨和最高降雨概率，快速找到周末更适合去的城市。`;
  return {
...
export default async function Page({
  ...
  const name = country.name["zh-cn"];
  const items = buildCountryWeekendWeatherRanking(dataset, country.slug, "zh-cn");
  const path = `/${country.slug}/best-weekend`;
