...
export async function generateMetadata({
  ...
  if (country === undefined) return { title: "國家週末旅行天氣" };
  const name = toTraditionalText(country.name["zh-cn"]);
  const path = `/${country.slug}/best-weekend`;
  const title = `${name}本週末哪裡天氣好？週六週日基本不下雨城市排行 | Where Not Rain`;
  const description = `比較${name}熱門旅行地即將到來的週六、週日基本不下雨天數、預計總降雨和最高降雨機率，快速找到週末更適合去的城市。`;
  return {
...
export default async function Page({
  ...
  const name = toTraditionalText(country.name["zh-cn"]);
  const items = buildCountryWeekendWeatherRanking(dataset, country.slug, "zh-hant");
  const path = `/${country.slug}/best-weekend`;
