...
const COPY = {
  en: {
    ...
    intro: "Keep up to five map states on this device, including dates, filters and compared destinations.",
...
export function CountrySavedViewsControl({
  ...
  useEffect(() => {
    try {
      setViews(parseSavedCountryMapViews(window.localStorage.getItem(COUNTRY_MAP_SAVED_VIEWS_STORAGE_KEY)));
    } catch {
  ...
  function persist(next: ReadonlyArray<SavedCountryMapView>): void {
    ...
    try {
      ...
      else window.localStorage.setItem(COUNTRY_MAP_SAVED_VIEWS_STORAGE_KEY, serializeSavedCountryMapViews(next));
    } catch {
      setStatus(copy.storageError);
    }
  }
  ...
  return (
    <>
      <div className="flex items-center gap-2">
        ...
        <button type="button" onClick={() => setOpen(true)} className="country-share-button focus-ring">
      ...
      {open ? (
        <div className="fixed inset-0 z-[190] bg-black/30 p-3 sm:p-6" role="presentation" onClick={() => setOpen(false)}>
          <section role="dialog" aria-modal="true" aria-label={copy.title} onClick={(event) => event.stopPropagation()} className="absolute inset-x-3 bottom-3 max-h-[76vh] overflow-y-auto rounded-3xl border border-border bg-white p-4 shadow-2xl sm:inset-x-6 sm:bottom-6 sm:p-6 lg:left-1/2 lg:right-auto lg:w-[min(680px,calc(100vw-3rem))] lg:-translate-x-1/2">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="eyebrow">{copy.title}</p>
                <p className="mt-2 max-w-xl text-sm text-muted">{copy.intro}</p>
              </div>
              <button type="button" onClick={() => setOpen(false)} aria-label={copy.close} className="rounded-full border border-border px-3 py-2 text-sm font-semibold focus-ring">×</button>
            </div>
            ...
            {views.length === 0 ? (
              ...
              <ul className="mt-6 grid gap-3">
                {views.map((view) => (
                  <li key={view.id} className="rounded-2xl border border-border bg-surface-elevated p-4">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div className="min-w-0">
                        <strong className="block truncate text-sm text-foreground">{view.label}</strong>
                        <span className="mt-1 block truncate text-xs text-muted">{view.url}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <button type="button" onClick={() => window.location.assign(view.url)} className="rounded-full bg-foreground px-3 py-2 text-xs font-bold text-white focus-ring">{copy.open}</button>
                        <button type="button" onClick={() => remove(view.id)} className="rounded-full border border-border px-3 py-2 text-xs font-semibold text-muted focus-ring">{copy.remove}</button>
                      </div>
                    </div>
                  </li>
                ))}
