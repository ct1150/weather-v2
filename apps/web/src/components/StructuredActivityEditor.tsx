...
const COPY = {
  en: {
    ...
    intro: "Mark what is indoors, outdoors or hard to move so weather alternatives stay practical.",
    migrated: "Your original itinerary text is kept when you edit these details.",
    addPoi: "Add place",
    choosePoi: "Choose a place",
    ...
    uv: "High UV risk",
    fixedWarning:
      "This activity is fixed or requires a reservation. Keep it unless the booking can be changed; use the alternatives as contingency options only.",
    alternatives: "Alternatives less affected by weather",
    addFallback: "Add as fallback",
    noPoi:
      "No suggested places are available for this city yet. You can still add your own backup.",
  },
  "zh-cn": {
    title: "当天活动",
    intro: "标记室内外、是否方便调整和预约情况，天气变化时更容易找到合适备选。",
    migrated: "编辑这些信息时，原来的行程文字会保留。",
    addPoi: "添加地点",
    choosePoi: "选择一个地点",
    quick: "快速添加活动",
    ...
    uv: "高紫外线风险",
    fixedWarning:
      "该活动是固定约束或必须预约。除非确认可以改期，否则不要移动；以下地点只作为备用方案。",
    alternatives: "更不受天气影响的备选",
    addFallback: "加入备选",
    noPoi: "这个城市暂时没有推荐地点，你仍可以自己添加备选。",
  },
  "zh-hant": {
    title: "當天活動",
    intro: "標記室內外、是否方便調整和預約情況，天氣變化時更容易找到合適備選。",
    migrated: "編輯這些資訊時，原來的行程文字會保留。",
    addPoi: "新增地點",
    choosePoi: "選擇一個地點",
    quick: "快速新增活動",
    ...
    uv: "高紫外線風險",
    fixedWarning:
      "該活動是固定約束或必須預約。除非確認可以改期，否則不要移動；以下地點只作為備用方案。",
    alternatives: "較不受天氣影響的備選",
    addFallback: "加入備選",
    noPoi: "這個城市暫時沒有推薦地點，你仍可以自己新增備選。",
  },
} as const;
...
function environmentLabel(
  value: TripActivityEnvironment,
  copy: (typeof COPY)[StructuredActivityLocale],
): string {
  return value === "indoor" ? copy.indoor : value === "outdoor" ? copy.outdoor : copy.mixed;
}

function flexibilityLabel(
  value: TripActivityFlexibility,
  copy: (typeof COPY)[StructuredActivityLocale],
): string {
...
export function StructuredActivityEditor({
  ...
  return (
    <section
      className="mt-5 rounded-2xl border border-border/80 bg-surface-elevated p-4"
      data-structured-activities="v2"
    >
      <div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        <div className="flex gap-2">
          ...
          />
          <button type="button" className="trip-secondary-button" onClick={addQuick}>
            {copy.add}
          </button>
        </div>
        <div className="flex gap-2">
          ...
          >
            ...
            {pois.map((poi) => (
              <option key={poi.id} value={poi.id}>
                {poiName(poi, poiLocale(locale))} · {environmentLabel(poi.environment, copy)}
              </option>
            ))}
          </select>
          <button
        ...
        {items.length === 0 ? (
          ...
          items.map((item, index) => (
            <article
              ...
              data-activity-environment={item.environment}
              data-activity-flexibility={item.flexibility}
            >
              <div className="grid gap-3 sm:grid-cols-[110px_1fr]">
                <label className="grid gap-1 text-xs font-semibold text-muted">
                  ...
                  <input
                    ...
                    className="min-h-10 rounded-lg border border-border px-2 text-sm text-foreground"
                    onChange={(event) => update(index, { startTime: event.target.value || null })}
                  />
                </label>
                <label className="grid gap-1 text-xs font-semibold text-muted">
                  {copy.name}
                  <input
                    ...
                    value={item.title}
                    className="min-h-10 rounded-lg border border-border px-2 text-sm text-foreground"
                    onChange={(event) => update(index, { title: event.target.value.slice(0, 180) })}
                  />
                </label>
              </div>
              <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
                <label className="grid gap-1 text-xs text-muted">
                  ...
                  <select
                    ...
                    onChange={(event: ChangeEvent<HTMLSelectElement>) =>
                      update(index, { environment: event.target.value as TripActivityEnvironment })
                    }
                  >
                    {(["indoor", "outdoor", "mixed"] as const).map((value) => (
                      <option key={value} value={value}>
                        {environmentLabel(value, copy)}
                      </option>
                    ))}
                  </select>
                </label>
                <label className="grid gap-1 text-xs text-muted">
                  ...
                  <select
                    ...
                    className="min-h-10 rounded-lg border border-border bg-white px-2 text-sm"
                    onChange={(event) =>
                      update(index, { flexibility: event.target.value as TripActivityFlexibility })
                    }
                  >
                    {(["fixed", "movable", "flexible"] as const).map((value) => (
                      <option key={value} value={value}>
                        {flexibilityLabel(value, copy)}
                      </option>
                    ))}
                  </select>
                </label>
                <label className="grid gap-1 text-xs text-muted">
                  ...
                  <select
                    ...
                    className="min-h-10 rounded-lg border border-border bg-white px-2 text-sm"
                    onChange={(event) =>
                      update(index, { priority: event.target.value as TripActivityPriority })
                    }
                  >
                    {(["must", "preferred", "optional"] as const).map((value) => (
                      <option key={value} value={value}>
                        {priorityLabel(value, copy)}
                      </option>
                    ))}
                  </select>
                </label>
                <label className="grid gap-1 text-xs text-muted">
                  ...
                  <select
                    ...
                    className="min-h-10 rounded-lg border border-border bg-white px-2 text-sm"
                    onChange={(event) =>
                      update(index, { reservation: event.target.value as TripActivityReservation })
                    }
                  >
                    {(["none", "recommended", "required"] as const).map((value) => (
                      <option key={value} value={value}>
                        {reservationLabel(value, copy)}
                      </option>
                    ))}
                  </select>
                </label>
              ...
              <div className="mt-3 flex items-center justify-between gap-3 text-[11px] text-muted">
                <span>
                  {item.poiId ?? "manual"} ·{" "}
                  {item.weatherSensitivity.length === 0
                    ? copy.indoor
                    : item.weatherSensitivity.join(" / ")}
                </span>
                <button
                  type="button"
      ...
      {planB !== null ? (
        <section
          ...
          data-concrete-plan-b="true"
        >
          <p className="eyebrow">{copy.planB}</p>
          <p className="mt-2 text-sm font-bold text-foreground">
            {copy.affected}: {planB.affectedActivity.title}
          </p>
          <p className="mt-1 text-xs text-muted">{copy[planB.reason]}</p>
          {planB.fixed ? (
            <p className="mt-3 rounded-lg bg-surface-elevated p-3 text-xs leading-5 text-foreground">
              {copy.fixedWarning}
            </p>
          ) : null}
          {planB.candidates.length > 0 ? (
            <>
              <p className="mt-4 text-xs font-bold text-muted">{copy.alternatives}</p>
              <div className="mt-2 flex flex-wrap gap-2">
                {planB.candidates.map((candidate) => (
                  <button
                    key={candidate.poi.id}
                    type="button"
                    className="trip-secondary-button"
                    onClick={() => addPoi(candidate.poi.id, true)}
                  >
                    {copy.addFallback}: {candidate.label}
                  </button>
                ))}
              </div>
            </>
          ) : (
            <p className="mt-3 text-xs text-muted">{copy.noPoi}</p>
          )}
        </section>
      ) : null}
    </section>
