...
const COPY = {
  en: {
    ...
    mapHeading: "City rain outlook at a glance",
    ...
    mapCount: (count: number) => `${count} cities`,
  ...
  "zh-cn": {
    ...
    copied: "链接已复制",
    copyFailed: "暂时无法复制",
    mapHeading: "城市降雨情况一张图查看",
    mapHint: (count: number) =>
      `地图只显示 ${count} 个按天气着色的地点圆点。鼠标移到圆点可快速查看，点击或轻触后看逐日预报。`,
    mapCount: (count: number) => `共 ${count} 个城市`,
    mapLegend: "地图天气图例",
    lowerRain: "少雨为主",
    mixed: "晴雨交替",
    rainLikely: "降雨偏多",
  ...
  "zh-hant": {
    ...
    copied: "連結已複製",
    copyFailed: "暫時無法複製",
    mapHeading: "城市降雨情況一張圖查看",
    mapHint: (count: number) =>
      `地圖只顯示 ${count} 個按天氣著色的地點圓點。滑鼠移到圓點可快速查看，點擊或輕觸後看逐日預報。`,
    mapCount: (count: number) => `共 ${count} 個城市`,
    mapLegend: "地圖天氣圖例",
    lowerRain: "少雨為主",
    mixed: "晴雨交替",
    rainLikely: "降雨偏多",
