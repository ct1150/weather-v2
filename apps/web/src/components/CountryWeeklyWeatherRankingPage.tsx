...
const COPY = {
  en: {
    eyebrow: "Country weekly weather ranking",
    title: (country: string) => `${country}: where has the best weather this week?`,
    intro: (country: string) =>
      `Compare supported destinations in ${country} by how many of the next seven forecast days are mostly rain-free, then by expected precipitation and peak rain chance.`,
    direct: (country: string, best: WeeklyWeatherRankItem | undefined) =>
      best === undefined
        ? `No ranked forecast is available for ${country} right now.`
        : `${best.cityName} currently ranks first in ${country}: ${best.rainFreeDays} of ${best.totalDays} forecast days are mostly rain-free, with ${best.totalRainMm ?? "—"} mm expected precipitation.`,
    ...
    countryMap: (country: string) => `Open ${country} weather map`,
    methodTitle: "How this country ranking works",
    method:
      "Cities rank first by the number of mostly rain-free forecast days, then by lower expected precipitation and lower peak rain chance. Rain, drizzle, showers, thunder, hail, snow and sleet never count as rain-free.",
    source: "Forecast source",
    updated: "Data updated",
  },
  "zh-cn": {
    eyebrow: "国家旅行天气周榜",
    title: (country: string) => `这周${country}哪里天气更好？`,
    intro: (country: string) =>
      `比较${country}已收录旅行地未来 7 天“基本不下雨”的天数，再结合预计总降雨和最高降雨概率排序。`,
    direct: (country: string, best: WeeklyWeatherRankItem | undefined) =>
      best === undefined
    ...
    ranking: (country: string) => `${country}未来 7 天天气更好的旅行地`,
    rainFree: (item: WeeklyWeatherRankItem) =>
      item.rainFreeDays === item.totalDays
    ...
    dates: "基本不下雨日期",
    details: "查看城市天气",
    countryMap: (country: string) => `返回${country}天气地图`,
  ...
  "zh-hant": {
    eyebrow: "國家旅行天氣週榜",
    title: (country: string) => `這週${country}哪裡天氣更好？`,
    intro: (country: string) =>
      `比較${country}已收錄旅行地未來 7 天「基本不下雨」的天數，再結合預計總降雨和最高降雨機率排序。`,
    direct: (country: string, best: WeeklyWeatherRankItem | undefined) =>
      best === undefined
    ...
    ranking: (country: string) => `${country}未來 7 天天氣更好的旅行地`,
    rainFree: (item: WeeklyWeatherRankItem) =>
      item.rainFreeDays === item.totalDays
        ? `${item.totalDays}天基本都不下雨`
    ...
    dates: "基本不下雨日期",
    details: "查看城市天氣",
    countryMap: (country: string) => `返回${country}天氣地圖`,
...
export function CountryWeeklyWeatherRankingPage({
  locale,
  countryName,
  countryPath,
  items,
      ...
      <section className="hero-panel !p-6 sm:!p-10">
        <div className="relative z-10">
          ...
          <p
            ...
            data-country-ranking-answer
          >
            {copy.direct(countryName, best)}
          </p>
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <a href={countryPath} className="country-detail-link focus-ring">
              {copy.countryMap(countryName)} <span aria-hidden="true">→</span>
            </a>
