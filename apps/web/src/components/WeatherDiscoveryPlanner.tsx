...
const COPY = {
  en: {
    ...
    intro:
      "Choose exact dates and the conditions you care about. Rankings, map markers and comparisons all use the same persisted forecast snapshot.",
    when: "Travel dates",
    from: "From",
    to: "To",
    ...
    tripIntro:
      "The selected date range is split into contiguous city blocks. This creates city/day scaffolding only—POIs come in Phase 7.",
    ...
    filtersShare: "Filters are stored in the URL so this comparison can be shared.",
    remove: "Remove",
  },
  "zh-cn": {
    eyebrow: "天气探索 2.0",
    title: "先看天气，再决定去哪里。",
    intro: "选择准确日期和你真正关心的天气条件。排行榜、地图和城市对比始终使用同一份持久化天气快照。",
    when: "出行日期",
    from: "开始",
    to: "结束",
    intent: "你最在意什么？",
    ...
    titleTrip: "天气优选行程",
    filtersShare: "筛选条件会写入 URL，可直接分享当前比较结果。",
    remove: "移除",
  },
  "zh-hant": {
    eyebrow: "天氣探索 2.0",
    title: "先看天氣，再決定去哪裡。",
    intro: "選擇準確日期和你真正關心的天氣條件。排行榜、地圖和城市比較始終使用同一份持久化天氣快照。",
    when: "出行日期",
    from: "開始",
    to: "結束",
    intent: "你最在意什麼？",
    ...
    titleTrip: "天氣優選行程",
    filtersShare: "篩選條件會寫入 URL，可直接分享目前比較結果。",
    remove: "移除",
  },
} as const;
...
function cityPath(locale: WeatherDiscoveryLocale, city: TripCityOption): string {
  const path = `/${city.countrySlug}/${city.citySlug}`;
...
function localizeCities(
  cities: ReadonlyArray<TripCityOption>,
  locale: WeatherDiscoveryLocale,
): ReadonlyArray<TripCityOption> {
  return locale === "zh-hant" ? cities.map(toTraditionalCity) : cities;
}

function localizeForecast(
  forecast: ReadonlyArray<TripForecastDay>,
  locale: WeatherDiscoveryLocale,
): ReadonlyArray<TripForecastDay> {
  return locale === "zh-hant" ? forecast.map(toTraditionalForecast) : forecast;
}
...
function resultTemp(result: DiscoveryCityResult): string {
  const low = result.metrics.averageMinC;
  const high = result.metrics.averageMaxC;
  return low === null || high === null ? "—" : `${low}°–${high}°`;
}

export function WeatherDiscoveryPlanner({ locale }: { readonly locale: WeatherDiscoveryLocale }): ReactElement {
  ...
  useEffect(() => {
    ...
    const selected = (search.get("cities") ?? "")
      .split(",")
      .filter(Boolean)
  ...
  useEffect(() => {
    ...
    void fetch(`${API_BASE}/api/v1/trip-cities?locale=${apiLocale}`)
      ...
      .then((payload) => {
        if (!active) return;
        setCities(localizeCities(payload.data?.items ?? [], locale));
      })
      .catch(() => {
        if (!active) return;
        setState("error");
  ...
  const loadForecast = useCallback(async (): Promise<void> => {
    ...
    const dates = discoveryDateRange(applied.from, applied.to);
    if (dates.length === 0) {
      setState("error");
    ...
    try {
      ...
      let freshness = "";
      let anyStale = false;
      const allItems: TripForecastDay[] = [];
      for (const cityBatch of chunks(cities, MAX_CITIES_PER_REQUEST)) {
        const search = new URLSearchParams({
          cityIds: cityBatch.map((city) => city.cityId).join(","),
          from: applied.from,
          to: applied.to,
          locale: apiLocale,
        });
        ...
        if (typeof snapshotId !== "string") throw new Error("FORECAST_SNAPSHOT_MISSING");
        if (expectedSnapshot !== null && expectedSnapshot !== snapshotId)
        ...
        allItems.push(...localizeForecast(payload.data?.items ?? [], locale));
      }
      setForecast(allItems);
  ...
  const selectedResults = useMemo(
    () =>
      shortlist
        .map((cityId) => results.find((result) => result.city.cityId === cityId))
  ...
  const mapMarkers = useMemo(
    () =>
      results.map((result) => ({
        ...
        score: result.score,
        theme: applied.intent,
      })),
    [applied.intent, locale, results],
  );

  useEffect(() => {
    if (state !== "ready") return;
    setShortlist((current) => current.filter((cityId) => resultIds.has(cityId)));
  }, [resultIds, state]);
  ...
  const updateNumber = (
    ...
    setDraft((current) => ({ ...current, [key]: numericInput(event.target.value) }));
  };
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {listDiscoveryIntents().map((intent) => (
                <button
                  ...
                  className={`min-h-11 rounded-full border px-4 text-sm font-semibold focus-ring ${draft.intent === intent ? "border-foreground bg-foreground text-white" : "border-border bg-white text-foreground"}`}
      ...
      {state === "ready" ? (
        <>
          <section className="mt-10" aria-labelledby="discovery-results">
            <div className="flex flex-wrap items-end justify-between gap-3">
              ...
              <p className="text-xs text-muted">
                ...
                {updatedAt.length > 0 ? ` · ${copy.refreshed} ${new Date(updatedAt).toLocaleString()}` : ""}
                {stale ? ` · ${copy.stale}` : ""}
              </p>
            </div>
                      ...
                      <article className="destination-card h-full">
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            ...
                            <h3 className="mt-1 text-xl font-bold text-foreground">
                              <a href={cityPath(locale, result.city)} className="focus-ring hover:text-primary">
                          ...
                          <div className="score-orbit">
                            <div>
                              <p className="text-lg font-bold leading-none text-foreground">{result.score}</p>
                        ...
                        <dl className="relative mt-4 grid grid-cols-2 gap-2 text-sm">
                          <div className="metric-block">
                            ...
                            <dd className="mt-1 font-bold text-foreground">
                              {formatMetric(result.metrics.maxRainProbability, "%")}
                            </dd>
                          </div>
                          <div className="metric-block">
                            <dt className="text-xs text-muted">{copy.tempMetric}</dt>
                            <dd className="mt-1 font-bold text-foreground">{resultTemp(result)}</dd>
                          </div>
                          <div className="metric-block">
                            <dt className="text-xs text-muted">{copy.windMetric}</dt>
                            <dd className="mt-1 font-bold text-foreground">
                              {formatMetric(result.metrics.maxWindKph, " km/h")}
                            </dd>
                          </div>
                          <div className="metric-block">
                            <dt className="text-xs text-muted">{copy.uvMetric}</dt>
                            <dd className="mt-1 font-bold text-foreground">
                              {formatMetric(result.metrics.maxUv, "")}
                            </dd>
                          </div>
                        </dl>
                        ...
                        <ul className="relative mt-4 flex flex-wrap gap-1.5">
                          {result.reasonCodes.slice(0, 4).map((reason) => (
                            <li key={reason} className="rounded-full border border-border px-2.5 py-1 text-[11px] font-semibold text-muted">
                        ...
                        <button
                          ...
                          onClick={() => toggleShortlist(result.city.cityId)}
                        >
          ...
          {mapMarkers.length > 0 ? (
            <ExplorerMap
              markers={mapMarkers}
              theme={INTENT_COPY[locale][applied.intent]}
          ...
          <section className="info-panel mt-10" aria-labelledby="discovery-compare">
            ...
            <h2 id="discovery-compare" className="section-title mt-2">{copy.compare}</h2>
            ...
            {selectedResults.length < 2 ? (
              ...
              <div className="mt-5 overflow-x-auto pb-2">
                <div className="grid min-w-[760px] gap-3" style={{ gridTemplateColumns: `repeat(${selectedResults.length}, minmax(180px, 1fr))` }}>
                  {selectedResults.map((result) => (
                    <article key={result.city.cityId} className="rounded-2xl border border-border bg-white p-4">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <p className="text-xs text-muted">{result.city.countryName}</p>
                          <h3 className="text-lg font-bold text-foreground">{result.city.cityName}</h3>
                        </div>
                        <button type="button" className="text-xs font-bold text-primary" onClick={() => toggleShortlist(result.city.cityId)}>
                          {copy.remove}
                        </button>
                      </div>
                      <p className="mt-3 text-3xl font-bold text-foreground">{result.score}</p>
                      <p className="text-xs text-muted">{copy.score}</p>
                      <dl className="mt-4 grid gap-2 text-xs">
                        <div className="flex justify-between gap-3"><dt>{copy.rainMetric}</dt><dd className="font-bold">{formatMetric(result.metrics.maxRainProbability, "%")}</dd></div>
                        <div className="flex justify-between gap-3"><dt>{copy.tempMetric}</dt><dd className="font-bold">{resultTemp(result)}</dd></div>
                        <div className="flex justify-between gap-3"><dt>{copy.windMetric}</dt><dd className="font-bold">{formatMetric(result.metrics.maxWindKph, " km/h")}</dd></div>
                        <div className="flex justify-between gap-3"><dt>{copy.uvMetric}</dt><dd className="font-bold">{formatMetric(result.metrics.maxUv, "")}</dd></div>
                      </dl>
                      <p className="mt-4 text-xs font-bold uppercase tracking-[0.08em] text-muted">{copy.forecast}</p>
                      <ul className="mt-2 grid gap-2">
                        {result.forecastDays.map((day) => (
                          <li key={`${day.cityId}-${day.date}`} className="rounded-xl bg-surface-elevated p-2 text-xs">
                            <div className="flex justify-between gap-2"><strong>{day.date.slice(5)}</strong><span>{day.condition}</span></div>
                            <div className="mt-1 flex justify-between gap-2 text-muted"><span>{day.temperatureMinC ?? "—"}°–{day.temperatureMaxC ?? "—"}°</span><span>{day.rainProbability ?? "—"}%</span></div>
                          </li>
                        ))}
                      </ul>
                    </article>
          ...
          <section className="info-panel mt-6" aria-labelledby="discovery-trip">
            ...
            <h2 id="discovery-trip" className="section-title mt-2">{copy.trip}</h2>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-muted">{copy.tripIntro}</p>
            <div className="mt-5 flex flex-wrap gap-3">
              <button type="button" className="trip-primary-button" disabled={selectedResults.length === 0} onClick={() => createTrip(false)}>
                {copy.create}
              </button>
              <button type="button" className="trip-secondary-button" disabled={selectedResults.length === 0} onClick={() => createTrip(true)}>
                {copy.append}
              </button>
              {tripReady ? (
