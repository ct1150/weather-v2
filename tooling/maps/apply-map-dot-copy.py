...
COPY_REPLACEMENTS = [
    ...
    ('dryDays: "Lower-rain days",', 'dryDays: "Rain outlook",'),
    ('lowerRain: "Mostly lower rain",', 'lowerRain: "Lower rain",'),
    ('mixed: "Mixed weather",', 'mixed: "Mixed conditions",'),
    (
    ...
    ('dryDays: "少雨天数",', 'dryDays: "少雨情况",'),
    ('lowerRain: "整体少雨",', 'lowerRain: "少雨为主",'),
    ('mixed: "晴雨混合",', 'mixed: "晴雨交替",'),
    (
    ...
    ('dryDays: "少雨天數",', 'dryDays: "少雨情況",'),
    ('lowerRain: "整體少雨",', 'lowerRain: "少雨為主",'),
    ('mixed: "晴雨混合",', 'mixed: "晴雨交替",'),
    (
...
new_functions = '''function lowerRainHeadline(summary: CitySummary, locale: ExplorerLocale): string {
...
pattern = re.compile(
    r"function rainLabel\(summary: CitySummary, locale: ExplorerLocale\): string \{.*?\n\}\n\nfunction markerDetail\(summary: CitySummary, locale: ExplorerLocale\): string \{.*?\n\}",
    re.S,
)
text, count = pattern.subn(new_functions, text, count=1)
if count != 1:
    raise SystemExit("rain summary function replacement failed")

fraction = "{selected.dryDays}/{selected.days.length}"
if fraction not in text:
    raise SystemExit("inspector fraction not found")
text = text.replace(fraction, "{lowerRainHeadline(selected, locale)}", 1)
explorer_path.write_text(text)

explorer_test_path = Path("apps/web/src/components/country-weather-explorer.test.tsx")
test = explorer_test_path.read_text()
test = test.replace(
    'label?.includes("Tokyo") && label.includes("3/7")',
    'label?.includes("Tokyo") && label.includes("3 of 7 days look relatively low-rain")',
)
test = test.replace(
    'label?.includes("Osaka") && label.includes("7/7")',
...
test = test.replace(
    ...
    'expect(screen.getByLabelText("Tokyo weather summary").textContent).toContain("2 of 3 days look relatively low-rain");',
)
explorer_test_path.write_text(test)

Path("apps/web/src/components/country-map-mobile-density.test.tsx").write_text(
